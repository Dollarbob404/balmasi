...
def generate_bank():
    ...
    # Super TAKI
    bank.add(Card("SuprTAKI", None, "TAKI"), 2)
...
class Card:
    ...
    def placeable(self, bank, current_color, current_effect, current_symbol):
        if '2+' == current_effect and not '2+' == self.symbol:
            return False
        if current_color == self.color or (current_symbol == self.symbol and current_effect != "TAKI"):
            return True
        if current_symbol in ["King"]:
            return True
        if self.name in ["SuprTAKI", "ColrChng"]:
...
class Player:
    ...
    def play(self, bank, current_color, current_effect, current_symbol):
        is_valid = False
        while not is_valid:
            is_valid = True
            print(f"Top card: {bank.look_card(-1)} Color: {current_color}")
            print(f"Your cards: {self.cards}")
            card_name = input("Select card: ")
            if card_name == "draw" or card_name == "end":
                return None
            card = self.cards.draw_specific(card_name)

            # later add rules that block playing when effect blocked (2+) (or do it in main loop)

            if card is None:
                is_valid = False
            elif not card.placeable(bank, current_color, current_effect, current_symbol):
                is_valid = False
                self.cards.add(card)
...
class Bank:
    ...
    def first(self):
        if self.bank.number_of_cards() == 0:
            while self.discard_pile.number_of_cards() > 1:
...
class Taki:
    def __init__(self):
        ...
        # cpu2 = Player("CPU2")
        # cpu3 = Player("CPU3")
        self.players = [player, cpu1]
        give_cards(self.bank, self.players)
        while self.bank.look_card(-1) is None or not self.bank.look_card(-1).symbol.isnumeric():
            self.bank.discard(self.bank.first())

        self.game_over = False
        self.current_player_index = 0
        self.play_direction = 1
        self.current_color = self.bank.look_card(-1).color
        self.current_symbol = self.bank.look_card(-1).symbol
        self.current_effect = ""
    ...
    def run(self):
        while not self.game_over:
            ...
            turn_over = False
            while not turn_over:
                played_card = current_player.play(self.bank, self.current_color, self.current_effect, self.current_symbol)
                turn_over = True
                if played_card is None and self.current_effect != "TAKI":
                    current_player.add_cards(self.bank.first())
                    break
                elif played_card is None:
                    # end of taki
                    self.current_effect = ""
                    played_card = self.bank.discard_pile.draw(-1)
                self.bank.discard(played_card)

                # Update current status
                self.current_symbol = played_card.symbol
                if played_card.color in ['Y', 'R', 'B', 'G']:
                    self.current_color = played_card.color

                if self.current_effect != "TAKI":
                # Deal with special cards
                    if played_card.symbol == "ChngDir":
                        self.play_direction = -self.play_direction
                    elif played_card.symbol == "Stop":
                        self.current_player_index += self.play_direction
                    elif played_card.symbol == "+" or played_card.symbol == "King":
                        turn_over = False
                    elif played_card.symbol == "TAKI":
                        self.current_effect = "TAKI"
                        turn_over = False
                else:
                    turn_over = False
